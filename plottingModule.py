...
def plot_return_hist(symbol, start, end, log_scale=True):
    ...
    For better visualization, y-axis is set to log scale (fat tails).
    ...
    try: 
        ...
        if save.lower() == 'y':
            ...
            fig.write_image(f"{current_dir}/{folder_name}/{symbol}_return-distribution_{first_date}-{end}_log-{log_scale}.png")
            fig.write_html(f"{current_dir}/{folder_name}/{symbol}_return-distribution_{first_date}-{end}_log-{log_scale}.html")
...
if __name__ == '__main__':
    ...
    plot_return_hist("GOOG", "1950-01-01", "2020-01-01", True)
